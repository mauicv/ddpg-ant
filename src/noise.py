...
class LinearSegmentNoise:
    ...
    def __init__(
        ...
        self.dt = dt
        self.g = tf.random.Generator.from_seed(1)
        self.prev_event = np.zeros(self.dim, dtype=np.float32)
        self.current = np.zeros(self.dim, dtype=np.float32)
    ...
    def __call__(self):
        p  = self.g.uniform([1], minval=0, maxval=1, dtype=tf.dtypes.float32)
